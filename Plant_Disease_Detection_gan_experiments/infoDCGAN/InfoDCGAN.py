...
class InfoDCGAN(object):
    ...
    def build_generator(self):
        ...
        G_layer2 = tf.nn.relu(tf.matmul(G_layer1, self.G_W2) + self.G_b2)
        G_layer2 = tf.layers.batch_normalization(G_layer2, training=self.training)
        G_layer2 = tf.reshape(G_layer2, [-1, 7, 7, 128])
    ...
    def build_discriminator_and_Q(self):
        ...
        Q_layer5_cat = tf.nn.softmax(Q_layer5[:, :self.c_cat])
        Q_layer5_cont = tf.nn.sigmoid(Q_layer5[:, self.c_cat:])
        Q_c_given_x = tf.concat([Q_layer5_cat, Q_layer5_cont], axis=1)
