...
G_W2 = tf.Variable(xavier_init([256, Img_height*Img_width]))
G_b2 = tf.Variable(tf.zeros(shape=[Img_height*Img_width]))

theta_G = [G_W1, G_W2, G_b1, G_b2]


Q_W1 = tf.Variable(xavier_init([Img_height*Img_width, 128]))
Q_b1 = tf.Variable(tf.zeros(shape=[128]))

Q_W2 = tf.Variable(xavier_init([128, classes]))
Q_b2 = tf.Variable(tf.zeros(shape=[classes]))
...
for it in range(1000000):
    if it % 1000 == 0:
        ...
        idx = np.random.randint(0, classes)
        c_noise = np.zeros([16, classes])
